...
import com.heluo.gmall.realtime.common.util.FlinkSourceUtil
...
trait BaseAPP {
  def start(port: Int, parallelism: Int, ckAndGroupID: String, topicName: String): Unit = {
    ...
    // 3.读取数据
    val kafkaSource = FlinkSourceUtil.getKafkaSource(ckAndGroupID, topicName)

    val stream = env.fromSource(kafkaSource, WatermarkStrategy.noWatermarks(), "kafka_source")
